...
const useWorkOrder = ({ mode, setAction, setTabIndex, setVisible }) => {
  ...
  const visiblePopUp = useSelector((state) => state.quickReporting?.visiblePopUp)
  ...
  useEffect(() => {
    ...
    setFormValue((prev) => ({
      ...
      scheduled_start: moment(data?.scheduled_start).format('YYYY-MM-DDTHH:mm'),
      scheduled_finish: moment(data?.scheduled_finish).format('YYYY-MM-DDTHH:mm'),
      actual_start: moment(data?.actual_start).format('YYYY-MM-DDTHH:mm'),
      actual_finish: moment(data?.actual_finish).format('YYYY-MM-DDTHH:mm'),
    }))
    ...
    Notification.fire({
      ...
      if (result.isConfirmed) {
        const modifiedFormData = {
          ...
          assign_person: values?.assign_person,
          lead: values?.lead,
