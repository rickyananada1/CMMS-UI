...
const useLaborForm = ({ mode, setAction }) => {
  ...
  const createLabor = useCreateLaborMultiple()

  const handleCreateLabor = async (values) => {
    const body = values.labors.map((item) => ({
      ...
      rate: Number(String(item?.rate ?? '0').replace(/,/g, '.') || 0),
      line_cost: Number(
        ((+item?.quantity || 0) * (+item?.rate || 1) * (+item?.regular_hours || 0)).toFixed(2),
      ),
    }))
