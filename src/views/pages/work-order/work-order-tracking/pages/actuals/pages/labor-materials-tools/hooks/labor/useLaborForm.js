...
import { useUpdateLabor, useGetLabor_, useGetDetailLabor } from '../../services/labor'
...
const useLaborForm = ({ mode, setAction }) => {
  ...
  const getDetailWorkOrderTaskLaborService = useGetDetailLabor({
    id: selectedWorkOrderRow?.work_order_id,
    labor_actual_id: selectedLaborRow?.work_order_labor_actual_id,
  })
  ...
  const handleUpdateLabor = async (values) => {
    ...
    await updateLabor
      .mutateAsync({
        ...
        labor_id: selectedLaborRow?.work_order_labor_actual_id,
      })
      .then((response) => {
