...
import useLocationMeterReading from './hooks/useLocationMeterReading'
import moment from 'moment'
import { measurementsSchema } from './schema'

const LocationMetersReading = () => {
  const { visible, setVisible, selectedLocationMeter, formValue, handleSubmit, isLoading } =
    useLocationMeterReading()
  return (
              ...
              <CModalBody>
                <div className="d-flex flex-column border-l-[4px] border-primary-main rounded-sm mb-2">
                  <p className="ms-2 font-semibold text-body-medium mb-0">
                    {selectedLocationMeter?.meter ?? '-'}
                  </p>
                  <p className="ms-2 text-body-medium mb-0">
                    {selectedLocationMeter?.meter_description ?? '-'}
                  </p>
                </div>
                <hr />
                {isLoading ? (
...
export default LocationMetersReading
