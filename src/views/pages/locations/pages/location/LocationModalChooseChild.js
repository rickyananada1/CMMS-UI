...
import useChooseChildLocation from './hooks/useChooseChildLocation'
...
const LocationModalChooseChild = ({
  mode,
  setAction,
  setTabIndex,
  ...
  return (
    <CModal
      alignment="center"
      size="lg"
      visible={visiblePopUpLocationChild}
      onClose={() => setVisiblePopUpLocationChild(false)}
    >
      <Formik enableReinitialize initialValues={formValue} onSubmit={handleSubmit}>
        {({ isSubmitting, values, setFieldValue }) => {
          return (
            <Form>
              <CModalHeader>
                <CModalTitle>Choose Child Location</CModalTitle>
              </CModalHeader>
              <CModalBody>
                ...
                <div className="d-flex flex-column border-l-[4px] border-orange-border rounded-sm mb-2">
                  <p className="ms-2 font-semibold mb-0">
                    You can’t assign location as a children for location that has another parent.
                  </p>
                </div>
...
export default LocationModalChooseChild
