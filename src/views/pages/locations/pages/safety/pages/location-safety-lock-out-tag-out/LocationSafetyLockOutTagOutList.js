...
import useLocationSafetyLockOutTagOutList from './hooks/useLocationSafetyLockOutTagOutList'
import {
  useDeleteLocationSafetyLockOutTagOut,
              ...
              <Fragment key={`tag-out-${item?.tag_out?.tag_out_id}`}>
                ...
                <div className="grid grid-cols-4 mb-3">
                  <div>
                    ...
                    <p className="text-body-bold">{selectedRow?.location ?? '-'}</p>
                  </div>
                </div>
...
const LocationSafetyLockOutTagOutList = ({
  mode,
  setAction,
  setTabIndex,
  isRefetchList,
  ...
  const {
    ...
    handleSearch,
    searchDebounce,
  } = useLocationSafetyLockOutTagOutList({
    mode,
    setAction,
    setTabIndex,
    isRefetchList,
  ...
  const columns = [
    ...
    },
    {
      ...
      columns: [
        ...
        {
          header: 'Location',
          accessorKey: 'tag_out_lock_outs_location',
          cell: ({ row }) => {
            const isHasLength =
              Array.isArray(row?.original?.tag_out_lock_outs) &&
              row?.original?.tag_out_lock_outs?.length
            ...
            return (
              <ol start={1} type="1" style={{ listStyleType: isHasLength ? 'decimal' : 'none' }}>
                {isHasLength ? (
                  row?.original?.tag_out_lock_outs.map((item, index) => {
                    return (
                      <Fragment key={`tag_out_location-${index}`}>
                        <li
                          key={`tag_out_location-${item?.tag_out?.tag_out_id}-${index}`}
                          className="truncate"
                        >
                        ...
                        {item?.lock_outs?.slice(0, -1)?.length > 0 &&
                          item.lock_outs.slice(0, -1).map((itemLock, indexLock) => (
                            <li
                              key={`tag_out_location-${item?.tag_out?.tag_out_id}-${index}-space-${indexLock}`}
                              className="truncate"
                            >
                              <span> </span>
                            </li>
    ...
    <div>
      <DetailCard>
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            onClick={downloadLocationSafetyLockOutTagOut}
          >
        ...
        <Table
          ...
          apiController={useGetLocationSafetyLockOutTagOutTableList}
          query={{
      ...
      {mode === 'Delete' && (
        <DeleteConfirmation
          ...
          deleteService={useDeleteLocationSafetyLockOutTagOut}
          tableRef={tableRef}
          deleteBody={{
...
export default LocationSafetyLockOutTagOutList
