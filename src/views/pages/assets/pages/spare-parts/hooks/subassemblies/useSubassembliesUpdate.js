...
const useSubassembliesUpdate = ({ setAction }) => {
  ...
  const handleUpdateSubassemblies = async () => {
    ...
    if (hasSameKey(textFields)) {
      ...
      const json = textFields.filter((item) => item.asset_id !== null)
      Notification.fire({
        icon: 'info',
            ...
            .then((response) => {
              if (response?.status === 200) {
                Notification.fire({
                  icon: 'success',
                  title: 'Success!',
                  text: `Subassemblies added successfully.`,
                }).then(() => {
                  setAction('Read')
                  setIsSubmitting(false)
                })
...
export default useSubassembliesUpdate
