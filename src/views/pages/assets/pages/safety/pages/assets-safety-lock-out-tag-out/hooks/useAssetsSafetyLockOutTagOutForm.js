...
import {
  ...
  useCreateAssetsSafetyLockOutTagOut,
  useUpdateAssetsSafetyLockOutTagOut,
...
const useAssetsSafetyLockOutTagOutForm = ({ mode, setAction, setTabIndex }) => {
  ...
  const getAssetsSafetyHazardsTagOut = useGetAssetsSafetyHazardsTagOut({
    id: selectedRow?.asset_id,
  })
  const getAssetsSafetyTagOut = useGetAssetsSafetyTagOut()

  const [formValue, setFormValue] = useState({
    safety_lock_out_tag_out: [
      {
        hazard: {
          value: '',
          label: '',
  ...
  const createAssetsSafetyLockOutTagOut = useCreateAssetsSafetyLockOutTagOut({
    id: selectedRow?.asset_id,
  })

  const getDetailAssetsSafetyLockOutTagOut = useGetDetailAssetsSafetyLockOutTagOut({
  ...
  const updateAssetsSafetyLockOutTagOut = useUpdateAssetsSafetyLockOutTagOut({
    id: selectedRow?.asset_id,
  })

  const getLockOutTagOutDetail = async () => {
    await getDetailAssetsSafetyLockOutTagOut
      .mutateAsync({
        id: selectedSafetyTagOutRow?.safety_lexicon_id,
        params: {
          site: selectedRow?.site_id,
    ...
    Notification.fire({
      ...
      if (result.isConfirmed) {
        if (mode === 'Create') {
          ...
          if (
            Array.isArray(values?.safety_lock_out_tag_out) &&
            values?.safety_lock_out_tag_out?.length
          ) {
            values?.safety_lock_out_tag_out.forEach((item) =>
          ...
          await createAssetsSafetyLockOutTagOut
            .mutateAsync({
              data: payload,
            })
            .then((res) => {
          ...
          await updateAssetsSafetyLockOutTagOut
            .mutateAsync({
              data: payload,
            })
            .then((res) => {
...
export default useAssetsSafetyLockOutTagOutForm
