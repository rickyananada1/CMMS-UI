...
import {
  ...
  useCreateAssetsSafetyHazardousMaterials,
  useUpdateAssetsSafetyHazardousMaterials,
...
const useAssetsSafetyHazardAndPrecautionsForm = ({ mode, setAction, setTabIndex }) => {
  ...
  const getAssetsSafetyHazards = useGetAssetsSafetyHazards()
  ...
  const createAssetsSafetyHazardousMaterials = useCreateAssetsSafetyHazardousMaterials({
    id: selectedRow?.asset_id,
  })

  const getDetailAssetsSafetyHazardousMaterials = useGetDetailAssetsSafetyHazardousMaterials({
  ...
  const updateAssetsSafetyHazardousMaterials = useUpdateAssetsSafetyHazardousMaterials({
    id: selectedRow?.asset_id,
  })

  const isLoading = getDetailAssetsSafetyHazardousMaterials.isPending

  const getHazardousMaterialDetail = async () => {
    await getDetailAssetsSafetyHazardousMaterials
      .mutateAsync({
        id: selectedSafetyMaterialRow?.safety_lexicon_id,
        params: {
          site: selectedRow?.site_id,
    ...
    Notification.fire({
      ...
      if (result.isConfirmed) {
        if (mode === 'Create') {
          ...
          if (Array.isArray(values?.hazardous_materials) && values?.hazardous_materials?.length) {
            values?.hazardous_materials?.forEach((item) =>
              hazards?.push({
          ...
          await createAssetsSafetyHazardousMaterials
            .mutateAsync({
              data: payload,
            })
            .then((res) => {
          ...
          await updateAssetsSafetyHazardousMaterials
            .mutateAsync({
