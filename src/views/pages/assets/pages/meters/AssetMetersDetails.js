...
const AssetMetersDetails = ({ row, CheckTag, XTag }) => {
    ...
    <CContainer className="my-3 px-5">
      {data?.finished_loading || data?.is_error ? (
        <Fragment>
          ...
          <CRow className="mt-4">
            <CCol>
              <CFormLabel>Life to Date for Asset</CFormLabel>
              <br />
              <span className="font-semibold">{data?.life_to_date_for_asset ?? '-'}</span>
            </CCol>
            <CCol>
...
export default AssetMetersDetails
