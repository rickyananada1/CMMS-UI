...
import useAssetMeter from './hooks/useAssetMeter'
...
import { assetMetersSchema } from './schema'

// eslint-disable-next-line react/prop-types
const AssetMetersForm = ({ mode, setAction, setTabIndex }) => {
  const {
    formValue,
    isLoading,
    getMeters,
  ...
  return (
    <Formik
      ...
      validationSchema={assetMetersSchema}
    >
      {({
        handleChange,
        setFieldValue,
                            ...
                            <div className="my-4">
                              ...
                              <div className="row">
                                ...
                                <div className="col">
                                  ...
                                  <Field
                                    ...
                                    onChange={async (val) => {
                                      if (
                                        !values.meter_in_groups.some(
                                          (e, i) =>
                                            e?.meter_id?.value === val?.value && i !== index,
                                        )
                                      ) {
                                        setFieldValue(`meter_in_groups.${index}.meter_id`, val)
                                        populateFields(setFieldValue, index, val)
                                      } else {
                                        setFieldValue(`meter_in_groups.${index}.meter_id`, null)
                                        duplicateMeterError()
                                      }
                                    }}
                                    onBlur={() =>
                              ...
                              <div className="row mt-2">
                                ...
                                <div className="col">
                                  ...
                                  <Field
                                    name={`meter_in_groups.${index}.life_to_date_for_asset`}
                                    placeholder="Enter Life to Date for Asset"
                                    value={item?.life_to_date_for_asset}
                                    invalid={
                                      touched?.meter_in_groups?.[index]?.life_to_date_for_asset &&
                                      errors?.meter_in_groups?.[index]?.life_to_date_for_asset
                                    }
                                    onChange={(e) => {
                                      setFieldValue(
                                        `meter_in_groups.${index}.life_to_date_for_asset`,
                                        e.target.value,
                                      )
                                    }}
                                    size="md"
...
export default AssetMetersForm
